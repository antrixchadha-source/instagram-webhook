...
// only engage on reels with an explicit per-post link override. Used by both
// the webhook (runtime routing) and the admin dashboard (display), so they
// agree on what's "the effective default link" for the account.
export const PER_POST_LINK_ONLY = new Set(["glide.explores"]);

export function effectiveAppLink(account) {
  if (!account) return null;
  if (PER_POST_LINK_ONLY.has(account.username)) return null;
  return account.app_link || null;
}
...
export async function upsertAccount(account) {
  ...
  const row = {
    ...
    app_link: account.app_link || null,
